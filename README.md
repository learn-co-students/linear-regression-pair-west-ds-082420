...
```python
#__SOLUTION__
# 1. Check for duplicate entries and NA's
...
#__SOLUTION__
print("""
            There are significant NA values in several features, particularly population,
            hepatitis_b, gdp, and GDP.
      """)

total_if_all_na_dropped = df.shape[0] - df.dropna().shape[0]

print(f"""
            If we dropped na's across all columns, 
            a total of {total_if_all_na_dropped} would be lost
       """)

print(f"""
            If we dropped all rows that contained na's, we would lose 
            {round(total_if_all_na_dropped/df.shape[0], 2)} of the data
        """)
```
